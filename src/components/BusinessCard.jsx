...
import { getBusinessDetails } from '../services/placesService';
...
const BusinessCard = ({ business, isReviewed = false, onToggleReviewed, customCategories = ['General'] }) => {
    ...
    const handleRevealPhone = async () => {
        ...
        try {
            const phone = await getBusinessDetails(id);
            setCurrentPhone(phone || 'No disponible');
            // Update business object locally if needed for context
            business.phone = phone;
        } catch (error) {
            console.error(error);
            setCurrentPhone('Error');
        } finally {
    ...
    const handleWhatsAppClick = () => {
        if (currentPhone && currentPhone !== 'No disponible' && currentPhone !== 'Error') {
            openWhatsApp(currentPhone, name);
        }
    };
            ...
            <div className="p-6">
                ...
                <div className="space-y-3 mb-6">
                    ...
                    <div className="flex items-center gap-3 h-8">
                        ...
                        {currentPhone ? (
                            <p className={`text-sm font-semibold tracking-wide ${currentPhone === 'No disponible' ? 'text-slate-500 italic' : 'text-slate-200'}`}>
                                {currentPhone}
                            </p>
                        ) : (
                            <button
                                ...
                                className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded-full font-bold transition-all shadow-lg shadow-blue-500/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                            >
                                {isLoadingPhone ? (
                                    <>
                ...
                <div className="flex gap-3 mt-auto">
                    <button
                        ...
                        disabled={!currentPhone || currentPhone === 'No disponible' || currentPhone === 'Error'}
                        className={`flex-1 px-4 py-3 font-bold rounded-lg transition-all duration-300 flex items-center justify-center gap-2 text-sm
                            ${currentPhone && currentPhone !== 'No disponible' && currentPhone !== 'Error'
                                ? 'bg-green-600 hover:bg-green-500 text-white shadow-[0_4px_12px_rgba(22,163,74,0.2)] hover:shadow-[0_8px_20px_rgba(22,163,74,0.3)] transform hover:-translate-y-0.5 active:translate-y-0'
                    ...
                    {mapsUrl && (
                        <a
                            ...
                            className={`${currentPhone ? '' : 'w-full'} px-4 py-3 bg-slate-800 hover:bg-slate-700 
